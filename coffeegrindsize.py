...
class coffeegrindsize_GUI:
	...
	def update_status(self, message):
		"""Helper to update status and refresh UI"""
	...
	def select_reference_object_mouse(self, event):
		...
		if self.img_source is None:
				...
				self.update_status("Original Image not Loaded Yet... Use Open Image Button...")
				return
		...
		#Update the user interface status
		self.update_status("Use your mouse to click on one side of the reference object... You will then need to click again to set up a measuring line...")
	...
	def select_region(self, event):
		...
		#Update the user interface status
		self.update_status("Click on the image to draw a polygon enclosing the coffee grounds, then press q...")
	...
	def quit_region_select(self, event):
		...
		if self.erase_clusters_mode is True:
			...
			#Update the user interface status
			self.update_status("The 'Erase Clusters' mode was deactivated.")
	...
	def change_reference_object(self, *args):
		...
		self.update_status("Reference Object set to "+self.reference_object.get()+"...")
	...
	def release_mouse(self, event):
		...
		if self.mouse_click_mode == "SELECT_REFERENCE_OBJECT":
			...
			self.update_status("The length of the the reference object was set to "+self.pixel_length_var.get()+" pixels... Now select its physical size with the library in the Reference Object dropdown menu, or with a custom Reference Physical Size...")
		...
		if self.mouse_click_mode == "SELECT_REFERENCE_OBJECT_READY":
			...
			#Refresh the user interface status
			self.update_status("Drag the red line across the reference object then click again... Be careful to exclude shadows, and make sure that the Angle of Reference Line entry is satisfactory to you...")
	...
	def move_start(self, event):
		...
		if self.erase_clusters_mode is True:
			...
			if (len(delclus) != 0) & (len(delclus) >= self.nclusters):
				...
				#Refresh the user interface status
				self.update_status("All clusters cannot be erased.")
				return
			...
			if (len(delclus) != 0) & (len(delclus) < self.nclusters):
				...
				#Refresh the user interface status
				self.update_status(str(len(delclus))+" clusters were erased.")
	...
	def downsample_image(self):
		...
		#Update the user interface status
		self.update_status("The Image was Downsampled by a Factor two... Current Size is ("+str(self.img.size[0])+", "+str(self.img.size[1])+")")
	...
	def open_image(self, event):
		...
		if image_filename != "":
			...
			#Refresh the user interface status
			self.update_status("Image opened: "+image_filename)
	...
	def threshold_image(self, event):
		...
		try:
			...
			#Update the user interface status
			self.update_status("Some Options in the User Interface are Invalid Numbers...")
			return
		...
		if self.polygon_alpha is not None:
			...
			if contained.size == 0:
				...
				#Refresh the user interface status
				self.update_status("No Image Pixels were Located Inside of the Analysis Region")
				return
		...
		if self.polygon_alpha is not None:
			...
			if contained.size == 0:
				...
				#Refresh the user interface status
				self.update_status("No Thresholded Pixels were Located Inside of the Analysis Region")
				return
		...
		#Refresh the user interface status
		self.update_status("Image thresholded: "+thresholded_fraction_str+"% of all pixels were thresholded")
	...
	def launch_psd(self, event):
		...
		if self.mask_threshold is None:
			...
			#Update the user interface status
			self.update_status("Image not Thresholded Yet... Use Threshold Image Button...")
			return
		...
		for i in range(nmask):
			...
			if i%10 == 0:
				...
				self.update_status("Iteration #"+str(i)+"; Fraction of thresholded pixels that were analyzed: "+frac_counted_str+" %")
		...
		self.update_status("Particle Detection Analysis Done ! Creating Cluster Map Image...")

		#Refresh cluster data
		self.refresh_cluster_data()

		#Refresh the user interface status
		self.update_status("Particle Detection Analysis Done ! Cluster Map Image is Now Displayed...")
	...
	def erase_clusters(self, event):
		...
		if self.nclusters is None:
			...
			#Update the user interface status
			self.update_status("Coffee Particles not Detected Yet... Use Launch Particle Detection Button...")
			return
		...
		if self.display_type.get() != outlines_image_display_name:
			...
			#Update the user interface status
			self.update_status("Please select 'Display Type' = 'Cluster Outlines' to use the 'Erase Clusters' tool.")
			return
		...
		if self.erase_clusters_mode is False:
			...
			#Update the user interface status
			self.update_status("Entered 'Erase Clusters' mode. Click with the mouse to erase all clusters within the circle. Zoom in or out to set precision. Hit Escape or the 'Erase Clusters' button to end.")
	...
	def create_histogram(self, event):
		...
		if self.pixel_scale_var.get() == "None":
			...
			#Update the user interface status
			self.update_status("The Pixel Scale Has Not Been Defined Yet... Use the Left Mouse Button to Draw a Line on a Reference Object and Choose a Reference Length in mm...")
			return
		...
		#Refresh the user interface status
		self.update_status("Histogram Image is Now Displayed...")
	...
	def save_data(self, event):
		...
		if self.nclusters is None:
			...
			#Update the user interface status
			self.update_status("Particles not Detected Yet... Use Launch Particle Detection Button...")
			return
		...
		#Update the user interface status
		self.update_status("Data Saved to "+filename+"...")
	...
	def save_histogram(self, event):
		...
		if self.img is None:
			...
			#Update the user interface status
			self.update_status("No View Created Yet...")
			return

		#Update the user interface status
		self.update_status("Saving View...")
		...
		#Update the user interface status
		self.update_status("Current View Saved to "+filename+"...")
